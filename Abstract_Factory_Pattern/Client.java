...
public class Client {
	...
	private void initialize() {
		...
		sword.setIcon(
				new ImageIcon(new ImageIcon("src/Images/diamondsword.png").getImage().getScaledInstance(80, 80, 0))); // creare
																														// buton
																														// pentru
																														// 'sword'
		...
		axe.setEnabled(false); // butoanele nu sunt setate pana cand nu se selecteaza un material
		...
		try {
			fabricaAbstracta = Selectare_Fabrica.selectare(material); // exceptie pentru a testa selectarea fabricii
		} catch (Exception e1) {
			item_craft.setEnabled(false);
		}
		try {
			fabricaAbstracta.create(item); // exceptie pentru a testa crearea item-ului
		} catch (Exception e2) {
			item_craft.setEnabled(false);
		}

		item_craft.addActionListener(new ActionListener() { // action listener pentru butonul de fabricare a item-ului
			public void actionPerformed(ActionEvent e) {

				System.out.println(item + " de " + material); // testare consola
				if (item.equals("sabie")) { // schimbare imagine in functie de item-ul creat
		...
		comboBox.addItemListener(new ItemListener() { // item listener pentru selectarea materialului
			public void itemStateChanged(ItemEvent e) {
				if (e.getStateChange() == ItemEvent.SELECTED) {
					System.out.println(comboBox.getSelectedItem());
					material = (String) comboBox.getSelectedItem();
					if (material != null) { // cand este selectat un material butoanele vor fi setate
						axe.setEnabled(true);
						pickaxe.setEnabled(true);
						sword.setEnabled(true);
					}
					if (material.equals("Fier")) { // schimbare imagini icon ale butoanelor
